<template>
  <DefaultLayout>
    <div class="px-10 py-10 flex flex-col">
      <div class="text-xl font-semibold">{{ orgInfo.name }} Donations</div>
      ...
      >
        ...
        >
          <thead
            class="text-lg text-white text-gray-700 bg-darkblue dark:text-gray-400"
          >
            <tr>
          ...
          <tbody>
            <tr
              v-for="donation in latestDonation"
              :key="donation.id"
            ...
            >
              ...
              <td class="py-4 px-6">{{ donation.amount }}</td>
              <td class="py-4 px-6">{{ getTimeDiff(donation.created) }}</td>
            </tr>
...
function loadLatestDonations() {
  ...
  api
    .get(`/donation/latest/${route.params.orgId}?page=${page.value}&size=10`)
    .then((response) => {
      response.data.forEach((element) => {
        latestDonation.value.push(element);
      });

      if (response.data.length == 0) {
        loadLabel.value["en"] = "No More Donations";
        loadLabel.value["bn"] = "No More Donations";
      } else {
